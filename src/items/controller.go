...
func (h handler) getItem(w http.ResponseWriter, r *http.Request) {
	...
	images, err := h.s.GetImages(itemId)
	if err != nil {
		responses.JSONError(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h handler) getItems(w http.ResponseWriter, r *http.Request) {
	...
	items, err := h.s.GetItems(limit, filter)
	if err != nil {
		responses.JSONError(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h handler) postItem(w http.ResponseWriter, r *http.Request) {
	...
	//checking the images before uploading them to the cloud
	...
	lastId, err := h.s.InsertItem(&item)
	if err != nil {
		responses.JSONError(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	for i := range files {
		...
		imageUrl, err := h.s.UploadItemImage(r.Context(), file)
		if err != nil {
			responses.JSONError(w, err.Error(), http.StatusInternalServerError)
			return
		}
		imageId, err := h.s.InsertItemImage(imageUrl)
		if err != nil {
			responses.JSONError(w, err.Error(), http.StatusInternalServerError)
			return
		}
		_, err = h.s.InsertItemImageJunction(item.Id, imageId, i)
		if err != nil {
			responses.JSONError(w, err.Error(), http.StatusInternalServerError)
			return
		}
...
func (h handler) updateItem(w http.ResponseWriter, r *http.Request) {
	...
	rowsAffected, err := h.s.UpdateItem(&item)
	if err != nil {
		responses.JSONError(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h handler) deleteItem(w http.ResponseWriter, r *http.Request) {
	...
	rowsAffected, err := h.s.DeleteItem(itemId)
	if err != nil {
		responses.JSONError(w, err.Error(), http.StatusInternalServerError)
		return
	}

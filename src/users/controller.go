...
func (h *handler) registerUser(w http.ResponseWriter, r *http.Request) {
	...
	userId, err := h.s.InsertUser(&user)
	if err != nil {
	...
	if err = h.s.CreateSession(userId, sessionId, string(claimsJson)); err != nil {
		responses.JSONError(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *handler) login(w http.ResponseWriter, r *http.Request) {
	...
	password, err := h.s.GetPasswordFromEmail(&userLogin)
	if err != nil {
	...
	claims, err := h.s.GetClaimsFromEmail(&userLogin)
	if err != nil {
		responses.JSONError(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	if err = h.s.CreateSession(claims.UserId, sessionUUID, string(claimsJson)); err != nil {
		responses.JSONError(w, err.Error(), http.StatusInternalServerError)
		return
	}
